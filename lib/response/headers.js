...
internals.absRegex = /^\w+\:\/\//;

exports.location = function (uri, server, request) {

    if (internals.absRegex.test(uri)) {
        return uri;
    }

    uri = uri.charAt(0) === '/' ? uri : '/' + uri;

    if (server.settings.location) {
        return server.settings.location + uri;
    }

    var scheme = server.info.protocol + '://';

    if (request) {
        if (request.info.host) {
            return scheme + request.info.host + uri;
        }
    }

    return scheme + server.info.host + ':' + server.info.port + uri;
};
